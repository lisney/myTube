...
### 순서 : (클라이언트)XMXHttpRequest 객체 생성/서버로 보냄 => (서버)해당 정보를 클라이언트에 보냄 => (클라이언트)받은 정보를 특정 영역에 뿌림
### XMLHttpRequest 메소드 : open(method, url, async) //method(GET/POST), url(서버/파일 위치), async(true/false)-동기/비동기
